...
from trainer import preprocessing_ops as preprocessing, layers
...
class Model:
    ...
    def encode(self, left, right):
        ...
        if FLAGS.dotprod:
            l_end = tf.nn.l2_normalize(l_encodings[-1], dim=3)
            r_end = tf.nn.l2_normalize(r_encodings[-1], dim=3)
            ...
            for l,r in zip(l_encodings, r_encodings):
